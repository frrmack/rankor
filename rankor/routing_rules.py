...
missing trailing slashes in the uri, instead of the default Werkzeug behavior
that Flask also repeats. 

The default Flask (and Werkzeug) behavior is that when it sees a url without a
trailing slash, it redirects to the same url with the trailing slash. This
...
is not a response this api wants to return, and 2) This is an api for machines
to interact with, not human users, and therefore it would rather be strict in
the format of requests it accepts (requiring the trailing slash), rather than
human friendly.
...
provide this redirect.) This keyword can be set to False to avoid the
redirection which is unwanted here. However, this results in behavior where the
server accepts uris with trailing slash AND those without it and return the same
response to either of them. As described above, this is not the behavior we want
in case of a missing trailing slash. We want to raise an exception in that case.
Therefore, we are updating the routing rule instead.
